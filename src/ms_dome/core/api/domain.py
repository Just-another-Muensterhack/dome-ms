...
from core.service import WebsiteManagementService
...
def list_domains(request: HttpRequest, website_id: UUID | None = None):
    return WebsiteManagementService(request.auth).list_domains(website_id)  # ty: ignore[unresolved-attribute]


@domains.get("/{domain_id}", response={200: DomainOut, 404: ErrorOut})
def get_domain(request: HttpRequest, domain_id: UUID):
    return WebsiteManagementService(request.auth).get_domain(domain_id)  # ty: ignore[unresolved-attribute]


@domains.post("/", response={201: DomainOut, 409: ErrorOut, 422: ErrorOut})
def create_domain(request: HttpRequest, payload: DomainIn):
    return 201, WebsiteManagementService(request.auth).create_domain(**payload.dict())  # ty: ignore[unresolved-attribute]
...
def update_domain(request: HttpRequest, domain_id: UUID, payload: DomainUpdate):
    service = WebsiteManagementService(request.auth)  # ty: ignore[unresolved-attribute]
...
def delete_domain(request: HttpRequest, domain_id: UUID):
    WebsiteManagementService(request.auth).delete_domain(domain_id)  # ty: ignore[unresolved-attribute]
    return 204, None
