...
def list_websites(request: HttpRequest):
    return WebsiteManagementService(request.auth).list_websites()  # ty: ignore[unresolved-attribute]


@websites.get("/{website_id}", response={200: WebsiteOut, 404: ErrorOut})
def get_website(request: HttpRequest, website_id: UUID):
    return WebsiteManagementService(request.auth).get_website(website_id)  # ty: ignore[unresolved-attribute]


@websites.post("/", response={201: WebsiteOut, 409: ErrorOut, 422: ErrorOut})
def create_website(request: HttpRequest, payload: WebsiteIn):
    return 201, WebsiteManagementService(request.auth).create_website(**payload.dict())  # ty: ignore[unresolved-attribute]
...
def delete_website(request: HttpRequest, website_id: UUID):
    WebsiteManagementService(request.auth).delete_website(website_id)  # ty: ignore[unresolved-attribute]
    return 204, None
